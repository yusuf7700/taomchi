...
// Barcha tasdiqlangan mahsulotlarning kalit so'zlaridagi alohida so'zlarni
// bitta to'plamga yig'adi (masalan "mol go'shti" -> "mol", "go'shti").
function buildCanonicalWordSet(canonicalItems) {
  const set = new Set();
  for (const item of canonicalItems) {
    for (const kw of (item.keywords || [])) {
      for (const w of String(kw).toLowerCase().split(/[^a-z']+/).filter(Boolean)) {
        set.add(w);
      }
    }
  }
...
// Bitta so'z hozir (joriy mahsulotlar ro'yxatiga ko'ra) allaqachon
// tanilganmi — eskirib qolgan takliflarni avtomatik tozalash uchun ishlatiladi.
function isWordCovered(word, canonicalWordSet) {
  return canonicalWordSet.has(String(word).toLowerCase());
}
