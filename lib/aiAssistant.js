...
const PREMIUM_IMAGE_DAILY_LIMIT = 5;
...
// Rasm bilan savol berish uchun alohida kunlik limit — faqat Premium
// foydalanuvchilarga ochiq, matn limitiga tegmaydi (alohida hisoblagich).
// Qaytaradi: { allowed, isPremium, remainingToday, reason }
// reason: "not_premium" | "limit" (faqat allowed=false bo'lganda mazmunli)
async function checkAndConsumeAiImageQuota(db, userId) {
  const userRef = db.collection("users").doc(String(userId));
  const today = getTashkentDateString();
  ...
  return db.runTransaction(async (tx) => {
    ...
    const isPremium = (data.premiumUntil || 0) > Date.now();
    if (!isPremium) {
      return { allowed: false, isPremium: false, remainingToday: 0, reason: "not_premium" };
    }

    const sameDay = data.aiImageUsageDate === today;
    const usedToday = sameDay ? (data.aiImageUsageCount || 0) : 0;

    if (usedToday >= PREMIUM_IMAGE_DAILY_LIMIT) {
      return { allowed: false, isPremium, remainingToday: 0, reason: "limit" };
    }

    tx.set(userRef, { aiImageUsageDate: today, aiImageUsageCount: usedToday + 1 }, { merge: true });
    return { allowed: true, isPremium, remainingToday: PREMIUM_IMAGE_DAILY_LIMIT - (usedToday + 1), reason: null };
  });
}
...
// images: ["data:image/jpeg;base64,...", ...] — ixtiyoriy, berilsa
// GPT-4o-mini bir nechta rasmni birgalikda ko'rib javob beradi (faqat
// Premium foydalanuvchilar uchun, eng ko'p 4 tagacha).
async function askFoodAssistant(question, lang, history = [], images = []) {
  ...
  const safeImages = Array.isArray(images) ? images.slice(0, 4) : [];

  const userContent = safeImages.length > 0
    ? [
        { type: "text", text: String(question).slice(0, 500) || "Rasmda nima ko'ryapsiz? Shundan qanday taom pishirsa bo'ladi?" },
        ...safeImages.map((url) => ({ type: "image_url", image_url: { url } }))
      ]
    : String(question).slice(0, 500); // haddan tashqari uzun matndan himoya
